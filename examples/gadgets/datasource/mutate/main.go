...
func do() error {
	...
	// executed before the cli operator
	...
	myOperator := simple.New("myHandler", simple.OnInit(func(gadgetCtx operators.GadgetContext) error {
		for _, d := range gadgetCtx.GetDataSources() {
			...
			// TODO: bool not supported yet
			isRootF, err := d.AddField("is_root", datasource.WithKind(api.Kind_Uint8))
			if err != nil {
			...
			d.Subscribe(func(source datasource.DataSource, data datasource.Data) error {
				...
				if uid == 0 {
					// TODO: yes, we know, it has to be improved.
					isRootF.Set(data, make([]byte, 1))
					isRootF.PutUint8(data, 1)
