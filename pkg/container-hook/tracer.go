...
import (
	...
	"github.com/inspektor-gadget/inspektor-gadget/pkg/utils/notify"
)
...
//
// Limitations:
// - the container runtime must be installed in one of the paths listed by notify.RuntimePaths
...
func (n *ContainerNotifier) install() error {
	...
	if runtimePath != "" {
		...
		notifiedPath, err := notify.Notify(runtimePath, host.HostRoot, runtimeBinaryNotify)
		if err != nil {
		...
		for _, r := range notify.RuntimePaths {
			log.Debugf("container-hook: trying runtime at %s", r)

			notifiedPath, err := notify.Notify(r, host.HostRoot, runtimeBinaryNotify)
			if err != nil {
	...
	if !runtimeFound {
		...
		return fmt.Errorf("no container runtime can be monitored with fanotify. The following paths were tested: %s. You can use the RUNTIME_PATH env variable to specify a custom path. If you are successful doing so, please open a PR to add your custom path to notify.RuntimePaths", strings.Join(notify.RuntimePaths, ","))
	}
