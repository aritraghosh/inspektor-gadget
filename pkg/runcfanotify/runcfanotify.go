...
func NewRuncNotifier(callback RuncNotifyFunc) (*RuncNotifier, error) {
	...
	if runcPath != "" {
		...
		notifiedPath, err := notify.Notify(runcPath, host.HostRoot, runcBinaryNotify)
		if err != nil {
		...
		for _, r := range notify.RuntimePaths {
			...
			notifiedPath, err := notify.Notify(r, host.HostRoot, runcBinaryNotify)
			if err != nil {
	...
	if !runcMonitored {
		...
		return nil, fmt.Errorf("no runc instance can be monitored with fanotify. The following paths were tested: %s. You can use the RUNC_PATH env variable to specify a custom path. If you are successful doing so, please open a PR to add your custom path to notify.RuntimePaths", strings.Join(notify.RuntimePaths, ","))
	}
